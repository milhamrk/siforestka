...
class Playlist extends CI_Controller {
	...
	public function detail(){
		$query = $this->model_utama->view_where('playlist',array('playlist_seo' => cetak($this->uri->segment(3))));
		if ($query->num_rows()<=0){
			redirect('main');
		}else{
			$row = $query->row_array();
	...
	public function watch(){
		$query = $this->model_utama->view_join_two('video','users','playlist','username','id_playlist',array('video.video_seo' => cetak($this->uri->segment(3))),'id_video','DESC',0,1);
		if ($query->num_rows()<=0){
			redirect('main');
		}else{
			$row = $query->row_array();
	...
	function kirim_komentar(){
		if (isset($_POST['submit'])){
			$cek = $this->model_utama->view_where('video',array('id_video' => cetak($this->input->post('a'))));
			$row = $cek->row_array();
			if ($cek->num_rows()<=0){
				redirect('main');
			}else{
