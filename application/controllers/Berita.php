...
class Berita extends CI_Controller {
	...
	function kirim_komentar(){
		if (isset($_POST['submit'])){
			$cek = $this->model_utama->view_where('berita',array('id_berita' => cetak($this->input->post('a'))));
			$row = $cek->row_array();
			if ($cek->num_rows()<=0){
				redirect('main');
			}else{
				if ($this->input->post() && (strtolower($this->input->post('secutity_code')) == strtolower($this->session->userdata('mycaptcha')))) {
					...
					$data = array('id_berita'=>cetak($this->input->post('a')),
		                            'nama_komentar'=>$usr['nama_lengkap'],
		                            'url'=>'#',
	...
	function indeks_berita(){
		...
		if (isset($_POST['filter'])){
			...
			if ($bulan <= 1){ $bulann = '0'.cetak($this->input->post('bulan')); }else{ $bulann = cetak($this->input->post('bulan')); }
			if ($tanggal <= 1){ $tanggall = '0'.cetak($this->input->post('tanggal')); }else{ $tanggall = cetak($this->input->post('tanggal')); }
			$fil = cetak($_POST['tahun']).'-'.$bulann.'-'.$tanggall;
			$data['hari_ini'] = $fil;
