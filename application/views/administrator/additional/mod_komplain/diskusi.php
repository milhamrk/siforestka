...
<div class="box">
  <div class="box-header">
    <h3 class="box-title">Resolution Center - <?php echo $rows['kode_transaksi']."/".$rows['id_reseller']; ?></h3>
  </div><!-- /.box-header -->
  <div class="box-body">
  <style>
	.user1 {
...
<?php 
  if ($detail['putusan'] == 'proses') {
      echo "<a style='text-align:left; margin-top:7px' class='btn btn-primary btn-block' href='" . base_url().$this->uri->segment(1) . "/room/" . $this->uri->segment(3) . "?status=selesai' onclick=\"return confirm('Apa anda yakin masalah ini sudah selesai?')\"><center>Selesai / Batalkan Komplain</center></a>";
  } else {
    echo "<div style='border-left:0px; margin-top:7px; padding:5px 10px' class='alert-success'><span class='fa fa-check-square'></span> Telah Selesai/Dibatalkan,..</div>";
  }
  echo "<hr><table class='table table-sm'>
          <tbody>
          <h4 class='no-margin'>No. Invoice</h4>    <p><a target='_BLANK' href='".base_url().$this->uri->segment(1)."/detail_penjualan_konsumen/$rows[id_penjualan]'>$rows[kode_transaksi]/$rows[id_reseller]</a></p>
          <h4 class='no-margin'>Lapak</h4>    <p><a target='_BLANK' href='".base_url()."u/$pen[user_reseller]'>$pen[nama_reseller]</a></p>
...
<div class="col-sm-9 col-lg-9 col-md-12 col-xs-12 col-12">
        <?php 
                        ...
                        if ($cek_respon->num_rows() >= 1) {
                          if ($cek_respon_pelapor->num_rows() <= 0) {
                            if (selisih_jam(date('Y-m-d H:i:s'), $rwork['waktu_kirim']) < 24) {
                              echo "<div class='refresh'><center>Menunggu Respon <b>$detail[pelapor]</b> (Pelapor) dalam
                                    ...
                                    <div id='form-comment'>
                                      ...
                                      <button class='btn btn-default refresh-btn pull-right'><span class='fa fa-refresh'></span></button>
                                      <button id='sendMessage' class='btn-sm btn-primary submitx' data-loading-text='Loading...'>Kirimkan</button>
                            ...
                            echo "<div id='form-comment'>
                                    <textarea class='form-control required textarea komentarx' id='comment' placeholder='Tuliskan Pesan Disini...' onkeyup=\"auto_grow(this)\" required></textarea>
                                    <button class='btn btn-default refresh-btn pull-right'><span class='fa fa-refresh'></span></button>
                                    <button id='sendMessage' class='btn-sm btn-primary submitx pull-right' data-loading-text='Loading...'>Kirimkan</button>
                                    <div id='mulitplefileuploader' class='mt-2'>Choose files</div>
                                    <div id='status'></div>
                                  </div>";
                          }
                          ...
                          if (selisih_jam(date('Y-m-d H:i:s'), $detail['waktu_report']) < 24) {
                            echo "<div class='refresh'><center>Menunggu Respon <b>$detail[terlapor]</b> (Terlapor) dalam
                                  ...
                                  <div id='form-comment'>
                                    <textarea class='form-control required textarea komentarx' id='comment' placeholder='Tuliskan Pesan Disini...' onkeyup=\"auto_grow(this)\" required></textarea>
                                    <button class='btn btn-default refresh-btn pull-right'><span class='fa fa-refresh'></span></button>
                                    <button id='sendMessage' class='btn-sm btn-primary submitx' data-loading-text='Loading...' style='float:right'>Kirimkan</button>
            ...
            if ($hitung_pesan->num_rows() > 10) {
              echo "<center><ul class='pagination messageatt'>
                  <li><a href='" . base_url() . $this->uri->segment(1). "/room/" . $this->uri->segment(3) . "/$prev' $prev_tombol>&lt; Selanjutnya</a></li>
                  <li><a href='" . base_url() . $this->uri->segment(1)."/room/" . $this->uri->segment(3) . "/$next' >Sebelumnya &gt;</a></li>
                </ul></center>";
            }
            echo "</div>";
        ?>
...
<script>
  $(document).ready(function() {
    var settings = {
      url: "<?php echo base_url().$this->uri->segment(1); ?>/upload_diskusi",
      formData: {
      ...
      deleteCallback: function(data, pd) {
        for (var i = 0; i < data.length; i++) {
          $.post("<?php echo base_url().$this->uri->segment(1); ?>/deleteFile_diskusi", {
              op: "delete",
              name: data[i]
            },
...
<script>
		$(document).ready(function() {
			...
			function tampilkan_diskusi() {
				...
				$('.messageatt').hide();
				$.ajax({
					url: '<?php echo site_url("administrator/read_query/" . $this->uri->segment(3) . '/' . $this->uri->segment('4')); ?>',
					type: 'GET',
					async: true,
					dataType: 'json',
              ...
              if (data[i].level=='admin'){
                ...
                '<td><a href="#"><b style="font-size:18px">' + nama_lengkap + '</b></a>, ' +
                '<span class="pull-right"><a style="margin-left:5px" href="javascript:void(0);" class="ps-product__remove diskusi_delete" style="cursor:pointer" data-id_diskusi="'+data[i].id_diskusi+'"><i style="color:red" class="fa fa-remove"></i></a></span>' +
      ...
      $('.tampilkan_diskusi').on('click','.diskusi_delete',function(){
        var id = $(this).data('id_diskusi');
          $.ajax({
              type : "POST",
              url  : "<?php echo site_url('administrator/diskusi_delete')?>",
              dataType : "JSON",
              data : {id:id},
              success: function(data){
                tampilkan_diskusi()
          ...
          return false;
			...
			$('#sendMessage').on('click', function() {
				...
				$.ajax({
					url: '<?php echo site_url("administrator/sendComment"); ?>',
					method: 'POST',
					data: {
						id: id,
						comment: comment
